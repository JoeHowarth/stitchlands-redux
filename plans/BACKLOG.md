...
- **Graphic-shadow material classification.** Commit 2 maps graphic shadow data to `MaterialKind::SunShadow` because the initial material table has no `GraphicShadow` variant. RimWorld separates graphic shadow data from projected static sun shadows; split or remap this when adding shadow material subkinds.

### Refactor / structure

- **Path-backed scene textures before shared `build_scene`.** Commit 2's first slice kept `SpriteInput`, `TexturedMeshInput`, and `EdgeSpriteInput` image-backed to keep the structural extraction reviewable. Before `plans/renderer-engine-boundary/` Commit 3, replace material backing images with path-backed scene texture records, move fog/snow material texture resolution to renderer ingest, and add the `TextureRegistry` path-cache test from `plan-v3.md`.
